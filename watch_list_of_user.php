...
$controller = ControllerFactory::getInstance()->getWatchController();
$data="";

$watchObj = new Watch;
$watchObj->processRequest();
// print_r($watchObj);
// die();
$watch_user_id= $watchObj->watch_user_id;

if(isset($watch_user_id) && $watch_user_id !=""){

	$resObj1 = $controller->listWatchsGivenMeForWatch($watch_user_id);
	// print_r($resObj1);exit;
		...
		foreach($resObj1 as $key =>$res){
			...
			///---///
			// echo $res->watch_user_id,$res->responsible_id;
			// exit;

			$resObj2 = $controller->listWatchsGivenByMEtoUser($watch_user_id,$res->assign_id,$res->responsible_id);
			// print_r($resObj2);
			// $resultArray[] = $resObj2;
	...
	if(!empty($obj_merged)){
		$arr_result = array(RESULT => SUCCESS,
							MESSAGE => "List of Watches given by me..!",
							DATA => $obj_merged);
	}else{
